...
if grep -E 'wheezy|jessie' /etc/os-release -qs; then
    ...
    wget -O /tmp/rtl-sdr.rules https://gh-proxy.com/https://github.com/HLLF-FAN/ADSB-scripts/blob/main/osmocom-rtl-sdr.rules
...
echo
echo "--------------------------------------------------------------------"
echo "站点经纬度已设置完成，readsb安装完成，开始安装tar1090"

cd "$ipath"

wget -O tar1090-install.sh https://gh-proxy.com/https://github.com/HLLF-FAN/tar1090-zh/raw/master/install.sh
bash tar1090-install.sh /run/readsb

rm -rf /etc/motd
wget -P /etc https://gh-proxy.com/https://github.com/HLLF-FAN/ADSB-scripts/blob/main/motd
